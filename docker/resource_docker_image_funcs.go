...
func resourceDockerImageCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if len(buildConfigList) == 1 {
		err := buildDockerImage(client, d.Get("name").(string), buildConfigList[0].(map[string]interface{}))
		if err != nil {
			return fmt.Errorf("Unable to build Docker image into resource: %s", err)
		}
